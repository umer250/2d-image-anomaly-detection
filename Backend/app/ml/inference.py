...
REAL_WORLD_THRESHOLD_FACTOR = 1.20
...
def run_inference(
    ...
    model_data = model_loader.get_model(category)
    base_threshold = float(model_data["threshold"])

    if remove_bg:
        threshold = round(base_threshold * REAL_WORLD_THRESHOLD_FACTOR, 6)
        print(f"[inference] Real-world mode: base_threshold={base_threshold:.4f} "
              f"× {REAL_WORLD_THRESHOLD_FACTOR} = threshold={threshold:.4f}")
    else:
    ...
    adaptive_sigma = max(2.0, 6.0 - anomaly_score * 2.0)
    anomaly_map_viz  = gaussian_filter(anomaly_map_viz, sigma=adaptive_sigma)
